...
class Session(object):
    ...
    def __init__(self, config=Config()):
        ...
        # self.parse_chart = self.playlist().parse_chart
    ...
    def get_featured(self):
        ...
        json_obj = self.request.request('GET', 'pages/home', params=params).json()

        tmpcharts = {}
        for i in json_obj['rows'][15]['modules'][0]['pagedList']['items']:
            tmpcharts[i['title']] = [i['uuid'], i['squareImage']]
        tmptracks = {}
        for i in json_obj['rows'][3]['modules'][0]['pagedList']['items']:
            tmptracks[i['title']] = [i['id'], i['album']['cover']]
        tmpalbums = {}
        for i in json_obj['rows'][2]['modules'][0]['pagedList']['items']:
            tmpalbums[i['title']] = [i['id'], i['cover']]
        
        tmpnews = {}
        for i in json_obj['rows'][-1]['modules'][0]['pagedList']['items']:
            tmpnews[i['title']] = [i['id'], i['cover']]
        tmpmixes = {}
        for i in json_obj['rows'][4]['modules'][0]['pagedList']['items']:
            tmpmixes[i['title']] = [i['id'], i['images']['SMALL']['url']]
        tmprecent = {}
        for i in json_obj['rows'][0]['modules'][0]['pagedList']['items']:
            typ = i['type']
            if typ == "ALBUM":
                tmprecent[i['item']['title']] = [i['item']['id'], i['item']['cover'], typ]
            else:
                tmprecent[i['item']['title']] = [i['item']['uuid'], i['item']['squareImage'], typ]

        result = {
            'mixes' : tmpmixes, # Mix (playlist/album?)
            'recent' : tmprecent, # Playlists (sometimes Albums)
            'charts' : tmpcharts, # Playlist
            'tracks' : tmptracks, # Track
            'albums': tmpalbums, # Album
            'news' : tmpnews # Album
        }
